import { IAssignment, IDataBase, ISchedule, IStudent } from "../types/ministry.types";
interface FetchOptions {
    method: string;
    //body: string;
}
const fetchData = async (url: string = '', data: IStudent[] | IAssignment[] | IStudent | ISchedule | ISchedule[] | IAssignment = [], method: string = 'GET') => {
...
export const lifeministry: any = function (this: IDataBase) {
    this.getStudents = function (gender: string = "all", active: string = "all") {
        ...
        return fetchData(stdUrl + "students?active=" + active + "&gender=" + gender);
    }
